...
public class UserDaoImpl implements UserDao {
	...
	public String signUp(User user) {
		...
		try {
			 ...
			 */
			mesg = "User signed up !" + user.getId();
...
//			user=session.get(User.class, userId);//reading from cache 
	...
	public List<User> getAllUsers() {
		...
		Transaction tx = session.beginTransaction();
		try {
			users = session.createQuery(jpql, User.class).getResultList();// select  //doubt
	...
	public User userSignIn(String email1, String password1) {
		...
		// user - DETACHED
		user.setPassword("asdfghjkl;");// modifying state of DETACHED entity -> changes not update in DB !
...
//		String jpql = "select u.firstName from User u where u.userRole=:role";
		String jpql="select u from User u inner join u.role r where r.role in :role";
		// 1. Get Session from SessionFactory
		Session session = getFactory().getCurrentSession();
		// 2. Begin Tx
		Transaction tx = session.beginTransaction();
		try {
			firstNames = session.createQuery(jpql, String.class)
	...
	public List<User> getSelectedUsersDetails(UserRole role1) {
		List<User> users = null;
		String jpql = "select new com.sunbeam.entities.User(u.firstName,u.lastName,u.dob) from User u where u.userRole=:role";
		// 1. Get Session from SessionFactory
		Session session = getFactory().getCurrentSession();
		// 2. Begin Tx
		Transaction tx = session.beginTransaction();
